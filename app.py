    ...
    with st.spinner("シミュレーション実行中..."):
        ...
        for idx, year in zip(year_indices, year_steps):
            ...
            if year == year_steps[0] and enable_crash:
                ...
                st.markdown("""
                    各シナリオを最終的な資産価値で分類し、それぞれの区分において途中で大きな下落を経験した割合を示しています:
                    
                    - 「上位5%の暴落経験率」は、最終的に最も高い資産価値(上位5%)を達成したシナリオのうち、
                      途中で{:.0f}%以上の下落を経験した割合です。
                    - 同様に「上位5-25%の暴落経験率」は、最終的に上位5-25%の資産価値となったシナリオの中で、
                      大きな下落を経験した割合を示します。
        ...
        for data in stats_data:
            ...
            if enable_crash:
                crash_info = {
                    "経過年数": data["経過年数"],
                    "上位5%の暴落経験率": data["上位5%の暴落経験率"],
                    "上位5-25%の暴落経験率": data["上位5-25%の暴落経験率"],
                    "上位25-50%の暴落経験率": data["上位25-50%の暴落経験率"],
                    "下位50-25%の暴落経験率": data["下位50-25%の暴落経験率"],
                    "下位25-5%の暴落経験率": data["下位25-5%の暴落経験率"],
                    "下位5%の暴落経験率": data["下位5%の暴落経験率"]
                }
